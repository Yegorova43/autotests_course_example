def repeats(our_str):
    """
    Считает, сколько раз каждый символ уже встречался в переданной строке
    :param our_str: строка, в которой надо подсчитать сколько раз каждый символ уже встречался
    :return: новая строка с символом и постфиксом в формате _n, который указывает на кол-во встречаний
    """
    #  создаем список для последовательного добавления каждого элемента и подсчета вхождения в этот список
    str_list = []
    new_str = ''
    for let in our_str:
        # если элемента нет в списке, сразу отдаем счетчик 1 и добавляем элемент в список
        if let not in str_list:
            new_str += str(let) + '_' + str(1)
            str_list.append(let)
        # если элемент уже есть в списке, считаем кол-во вхождений и прибавляем 1, добавляем элемент в список
        else:
